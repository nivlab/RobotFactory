...
## Compute trial-history effects.
roll = lambda x: np.roll(x, 1)
data['prev_sham'] = data.groupby(['subject','session','rune']).sham.transform(roll)
...
data['prev_sham'] = data.groupby(['subject','session']).prev_sham.transform(demean).fillna(0)
...
if endogenous == 1:
    main_effects = ['GW','NGW','GAL','NGAL','prev_sham']
elif endogenous == 2:
    main_effects = ['intercept','valence','action','interaction_1','prev_sham']
elif endogenous == 3:
    main_effects = ['intercept','valence','incongruent','interaction_2','prev_sham']
else:
