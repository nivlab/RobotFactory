...
parameters {
    ...
    vector[3] mu_pr;                   // Group-level parameters
    vector[3]  sigma_pr;               // Group-level variance

    matrix[3, max(S)]  theta_pr;       // Subject-level parameters
    cholesky_factor_corr[3]  L;        // Subject-level covariance
...
transformed parameters {
    ...
    matrix[max(S),3] theta = transpose(rep_matrix(mu_pr, max(S)) + diag_pre_multiply(exp(sigma_pr), L) * theta_pr);
