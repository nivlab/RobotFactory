data {
    ...
    // Task variables
    vector[N]  R[M];                   // Rewards (Reinforced = 1, Punished = 0)
...
transformed parameters {
    ...
    matrix[max(J),5] theta = transpose(rep_matrix(mu_pr, max(J)) + diag_pre_multiply(exp(sigma_pr), L) * theta_pr);
    
    // Extract random effects
    b0 = theta[,1] * 5;
    b1 = theta[,2] * 5;
    b2 = theta[,3] * 5;
    a1 = Phi_approx(theta[,4] + theta[,5]);
    a2 = Phi_approx(theta[,4] - theta[,5]);
    a3 = Phi_approx(theta[,4]);
