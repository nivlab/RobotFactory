...
parameters {
    ...
    vector[3]     theta_mu;                 // Population-level effects
    matrix[3,NJ]  theta_pr;                 // Standardized subject-level effects
    
    // Paramter variances
    vector<lower=0>[3] sigma;               // Subject-level standard deviations
...
transformed parameters {
    ...
    matrix[NJ,3] theta = transpose(diag_pre_multiply(sigma, theta_pr));
    
    // Construct random effects
    b1 = (theta_mu[1] + theta[,1]) * 10;
    b2 = Phi_approx(theta_mu[2] + theta[,2]);
    a1 = Phi_approx(theta_mu[3] + theta[,3]);
