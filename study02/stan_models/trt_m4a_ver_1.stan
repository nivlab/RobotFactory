...
data {
    ...
    vector[N] C[2,E];                  // Choice (Observed = 1, Censored = 0)      
...
parameters {
    ...
    matrix[5,max(S)]  theta_c_pr;
    matrix[5,max(S)]  theta_d_pr;
...
transformed parameters {
    ...
    vector[max(S)]  beta_GO[2];             // GO bias
    
    // Construction block
    {
    
    // Rotate random effects
    matrix[5,max(S)]  theta_c = diag_pre_multiply( exp(sigma_c_pr), L_c ) * theta_c_pr;
    matrix[5,max(S)]  theta_d = diag_pre_multiply( exp(sigma_d_pr), L_d ) * theta_d_pr;
...
model {
    ...
    for (i in 1:2) {
        ...
        vector[N]  beta_GO_i = beta_GO[i][S];
        ...
        for (e in 1:E) {
            ...
            // Define bias
            vector[N] bias =  beta_GO_i + (V[i, e] .* beta_P_i);
